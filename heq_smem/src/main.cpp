...
int main( int argc, const char** argv ) {
	...
	cudaMallocHost((void**)&imgGPU,height*width*sizeof(unsigned char));
	memcpy(imgGPU,img_hist_equalized_gpu.data,height*width*sizeof(unsigned char));        

	start_gpu = CLOCK();

        histogram_gpu((unsigned char *) imgGPU, 
                                        height, 
                                        width);

        finish_gpu = CLOCK();

	memcpy(img_hist_equalized_gpu.data,imgGPU,height*width*sizeof(unsigned char));
	cudaFreeHost(imgGPU);
